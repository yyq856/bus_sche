...
def create_model_parameters(solver_input):
    ...
    params['FC_r'] = 200
    params['VC_r'] = 30
    headway_minutes = [5, 10, 15, 20]
    params['J_r'] = {f'k{i}': hw / 60.0 for i, hw in enumerate(headway_minutes)}
    params['H_k'] = params['J_r']
    T_op = 4
    params['C_op_rk_template'] = {k: (T_op / h) * params['VC_r'] for k, h in params['H_k'].items()}
    params['C_op_env_hat'] = {'D': 30, 'X': 35, 'bg': 20}
    params['beta_TT_m'] = {
        'D': -3.0,  # 私家车在途时间以及费用
        'X': -3.5,  # 出租车在途时间以及费用
        'B': -0.8,  # 公交在途时间
        'R': -0.5,  # 地铁在途时间
        'W': -2.0  # 步行在途时间 (最辛苦，尤其长距离)
    }
    params['beta_WT'] = -1.0
    params['beta_TC'] = -0.2
    params['beta_AT'] = -0.8
    params['beta_const'] = {
        'D': 0.0,  # 私家车：提供便利性、隐私性等正面效用
        'X': -0.2,  # 出租车：没有私家车的便利性，作为中性基准
        'B': -0.8,  # 公交：需要走到车站等不便之处
        'R': -0.4,  # 地铁：可靠性高，不受拥堵影响
        'W': -2.0,  # 步行：只适用于短途，受天气影响
        'O': -3.0}
